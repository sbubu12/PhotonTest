...
public class AvatarController : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    private void Update()
    {
        if (photonView.IsMine)
        {
            ...
            if (input.sqrMagnitude > 0f)//これはなんだ
            {
                // 入力があったら、スタミナを減少させる
                currentStamina = Mathf.Max(0f, currentStamina - Time.deltaTime);
                transform.Translate(6f * Time.deltaTime * input.normalized);　//input.normalized?
            }
            else
            {
                // 入力がなかったら、スタミナを回復させる
                currentStamina = Mathf.Min(currentStamina + Time.deltaTime * 2, MaxStamina);//これはなに
            }
        }
